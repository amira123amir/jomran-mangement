...
export default function CustomNotesSection({ persona, role }: CustomNotesSectionProps) {
  ...
  const currentIds = getCurrentUserIds(persona);

  const myNotes = orders.flatMap(o =>
    o.customNotes
      .filter(n =>
        n.type === 'general'
          ? currentIds.includes(n.targetUserId)
          : (n.senderName === persona || currentIds.includes(n.targetUserId))
      )
      .map(n => ({ note: n, order: o }))
  ).sort((a, b) => b.note.createdAt.localeCompare(a.note.createdAt));

  const unreadCount = myNotes.filter(x => !x.note.isRead).length;
      ...
      {myNotes.length === 0 ? (
        ...
        <div className="custom-notes-list">
          {myNotes.map(({ note, order }) => (
            <div key={note.id} className={`custom-note-card ${note.isRead ? '' : 'unread'}`}>
