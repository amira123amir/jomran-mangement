...
export default function SalesOrderScreen() {
  ...
  const myOrders = allOrders.filter((o) => o.salesPersona === persona.name);
  const actionNeededOrders = myOrders.filter((o) => ['priced', 'pending_sales_info', 'revision'].includes(o.status));
  ...
  const handleSubmit = () => {
    ...
    if (!quantity.trim() || Number(quantity) <= 0) { setValidationError('يجب إدخال كمية صحيحة.'); return; }
  ...
  return (
    <div className="sos-screen">
      ...
      {orderSubmitted && (
        <div className="sos-success-overlay">
          <div className="sos-success-card">
            <div className="sos-success-icon">✅</div>
            <div className="sos-success-title">تم إرسال الطلب بنجاح</div>
            <div className="sos-success-num">رقم الطلب: #{lastOrderNum} | رمز الشحن: {lastShippingMark}</div>
          </div>
        </div>
      )}
          ...
          {myOrders.length === 0 ? (
            ...
            <>
                  ...
                  {actionNeededOrders.map((order) => (
                    <div key={order.id} className="sos-pricing-card" onClick={() => handleOpenWorkspace(order.id)}>
                      <div className="sos-pricing-main">
                        ...
                        <span className={`sos-order-status status-${order.status}`}>
                          {order.status === 'priced' ? '💰 تم التسعير — راجع' :
                            order.status === 'pending_sales_info' ? '🔴 بانتظار معلومات المبيعات' :
                              '🔄 مراجعة مطلوبة'}
                        </span>
                        {persona.department === 'sales' && (
                          <button className="sos-detail-btn" onClick={(e) => { e.stopPropagation(); setExpandedOrderId(expandedOrderId === order.id ? null : order.id); }}>
                      ...
                      {order.pricingHistory?.length ? (() => { const lp = order.pricingHistory[order.pricingHistory.length - 1]; return (
                        <div className="sos-pricing-details">
                          <span className="sos-pricing-usd">الإجمالي: ${lp.totalUSD.toFixed(3)}</span>
                        ...
                        <div className="sos-order-expanded">
                          <div className="sos-order-meta">
                            {[
                              ...
                              { label: 'السعر المستهدف', value: order.targetPrice !== undefined ? `$${order.targetPrice.toFixed(3)}` : '—' },
                              { label: 'السعر المعتمد', value: (() => { const lp = order.pricingHistory?.length ? order.pricingHistory[order.pricingHistory.length - 1] : null; return lp ? `$${lp.totalUSD.toFixed(3)}` : '—'; })() },
                              { label: 'اسم المعمل', value: order.supplierData?.factoryName || '—' },
                              { label: 'رقم المورد', value: order.supplierData?.supplierNumber || '—' },
                            ].map((f, i) => (
              ...
              <div className="sos-all-orders">
                <h3 className="sos-all-orders-title">جميع طلباتي ({myOrders.length})</h3>
                {myOrders.map((order) => (
                  <div key={order.id}>
                    <div className="sos-order-row" onClick={() => handleOpenWorkspace(order.id)}>
                      <span className="sos-order-num">#{order.orderNumber}</span>
                      <span className="sos-order-client">{order.clientName}</span>
                      <span className="sos-order-persona">{order.salesPersona}</span>
                      <span className={`sos-order-status status-${order.status}`}>
                              ...
                              order.status === 'priced' ? '💰 تم التسعير' :
                                ...
                                order.status === 'pending_factory_info' ? '🏭 بانتظار معلومات المصنع' :
                                  order.status === 'locked' ? '🔒 مقفل' :
                                    order.status === 'deposit_received' ? '💳 تم استلام الدفعة' :
                                      '✅ مكتمل'}
                      </span>
                      <SalesAgeCell createdAt={order.createdAt} />
                      {persona.department === 'sales' && (
                        <button className="sos-detail-btn" onClick={(e) => { e.stopPropagation(); setExpandedOrderId(expandedOrderId === order.id ? null : order.id); }}>
                          {expandedOrderId === order.id ? '⬆ إخفاء' : '📋 عرض التفاصيل الكاملة'}
                        </button>
                      )}
                    </div>
                    {expandedOrderId === order.id && (
                      <div className="sos-order-expanded">
                        <div className="sos-order-meta">
                          {[
                            ...
                            { label: 'الكمية', value: order.optionalFields?.quantity || '—' },
                            { label: 'السعر المستهدف', value: order.targetPrice !== undefined ? `$${order.targetPrice.toFixed(3)}` : '—' },
                            { label: 'السعر المعتمد', value: (() => { const lp2 = order.pricingHistory?.length ? order.pricingHistory[order.pricingHistory.length - 1] : null; return lp2 ? `$${lp2.totalUSD.toFixed(3)}` : '—'; })() },
                            { label: 'اسم المعمل', value: order.supplierData?.factoryName || '—' },
                            { label: 'رقم المورد', value: order.supplierData?.supplierNumber || '—' },
                          ].map((f, i) => (
                            <span key={i} className="sos-order-meta-item"><strong>{f.label}:</strong> {f.value}</span>
                        ...
                        <div className="sos-order-notes">
                          <strong>الملاحظات ({order.notes.filter((n) => {
                            if (n.targetPersona === ALL_PEOPLE) return true;
                            if (n.authorPersona === persona.name) return true;
                            if (n.targetPersona === persona.name) return true;
                            if (persona.name === 'محمد جمران') return true;
                            if (n.targetPersona === 'مديرة المشتريات كنان' && persona.department === 'procurement') return true;
                            return false;
                          }).length}):</strong>
                          {order.notes.filter((n) => {
                            if (n.targetPersona === ALL_PEOPLE) return true;
                            if (n.authorPersona === persona.name) return true;
                            if (n.targetPersona === persona.name) return true;
                            if (persona.name === 'محمد جمران') return true;
          ...
          <div className="sos-form">
            ...
            <div className="sos-section">
              ...
              <div className="sos-field" style={{ marginBottom: 14 }}>
                ...
                <input className="sos-input" type="text" inputMode="numeric" value={quantity} onChange={(e) => { setQuantity(e.target.value.replace(/[٠-٩]/g, (d) => String('٠١٢٣٤٥٦٧٨٩'.indexOf(d)))); setValidationError(''); }} placeholder="0" min="1" />
              </div>
